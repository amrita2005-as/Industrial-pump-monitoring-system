...
with kpi_col1:
    ...
    <div class="tooltip">
        ...
        <span class="tooltiptext">The maximum efficiency value achieved across all selected pumps in the chosen time range. Higher efficiency indicates better energy conversion and optimal performance.</span>
    </div>
    """, unsafe_allow_html=True)
    st.metric("", f"{df_combined['Efficiency_pct'].max():.2f}%", delta="Combined", label_visibility="collapsed")

with kpi_col2:
    st.markdown("""
    <div class="tooltip">
        <span>MIN EFFICIENCY<span class="info-icon">i</span></span>
        <span class="tooltiptext">The minimum efficiency value recorded across all selected pumps. This identifies periods of suboptimal performance requiring investigation.</span>
    </div>
    """, unsafe_allow_html=True)
    st.metric("", f"{df_combined['Efficiency_pct'].min():.2f}%", delta="Combined", label_visibility="collapsed")
